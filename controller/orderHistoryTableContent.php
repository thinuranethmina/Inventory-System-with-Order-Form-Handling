...
if (User::is_allow()) {
    ...
    if (isset($_POST['product'])) {
        if ($_POST['product'] != 0 && $_POST['product'] != '' && $_POST['product'] != null) {
            if ($isTextEmpty) {
                $order_rs = Database::search($sql2 . " GROUP BY `invoice`.`id`  ORDER BY `invoice`.`date_time` DESC");
            } else {
                $order_rs = Database::search($sql2 . " GROUP BY `invoice`.`id`  ORDER BY `invoice`.`date_time` DESC", "ssssss", ['%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%']);
            }
        } else {
            if ($isTextEmpty) {
            ...
            ?>
                    <span class="bg-white rounded rounded-3 mt-2 mr-1 px-3 py-2">
                        ...
                        if ($isTextEmpty) {
                            ...
                            $stock_rs = Database::search($sql2 . " GROUP BY `invoice`.`id`  ORDER BY `invoice`.`date_time` DESC", "ssssss", ['%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%', '%' . $_POST['text'] . '%']);
                        }
